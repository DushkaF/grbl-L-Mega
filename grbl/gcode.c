    ...
    case 'G':
      ...
      case 33: //Check if Synchronization pulses per revolution is set and hardware limits are enabled
        if (settings.sync_pulses_per_revolution == 0)
          FAIL(STATUS_GCODE_G33_SYNCHRONIZATION_PULSES_PER_REVOLUTION_NOT_SET);
        if (bit_isfalse(settings.flags, BITFLAG_HARD_LIMIT_ENABLE))
          FAIL(STATUS_GCODE_G33_HARDWARE_LIMITS_NOT_ENABLED);
        // No break. Continues to next line.
      case 0:
      case 1:
      case 2:
...
// [9. Override control ]: Not supported except for a Grbl-only parking motion override control.
  ...
  else if (axis_command == AXIS_COMMAND_MOTION_MODE)
  {
    ...
    else
    {
      ...
      {
      case MOTION_MODE_SPINDLE_SYNC:    //TORESEARCH
        if bit_isfalse (value_words, bit(WORD_K))
        {
          FAIL(STATUS_GCODE_VALUE_WORD_MISSING);
        } // [K value not given]
        else if bit_istrue (value_words, bit(WORD_F))
        {
          FAIL(STATUS_GCODE_UNUSED_WORDS);
        } // [F value given, but not needed], the check on other unused words will be done at the end
        else if (gc_block.modal.units == UNITS_MODE_INCHES)
        {
          gc_block.values.ijk[Z_AXIS] *= MM_PER_INCH;
        }                                    //calculate for inches
  ...
  */
  ...
  // [4. Set spindle speed ]:     // * TORESEARCH
...
// [9. Override control ]: NOT SUPPORTED. Always enabled. Except for a Grbl-only parking control.
  ...
  if (gc_state.modal.motion != MOTION_MODE_NONE)
  {
    if (axis_command == AXIS_COMMAND_MOTION_MODE)
    {
      ...
      else if (gc_state.modal.motion == MOTION_MODE_SPINDLE_SYNC)   //* TORESEARCH
      {
        ...
        { // There are synchronization pulses so wait for the next synchronization pulse

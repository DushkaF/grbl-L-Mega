...
void protocol_exec_rt_system()
{
  ...
  //depending on settings, reports are triggered. // * TORESEARCH
  if (bit_istrue(threading_exec_flags, EXEC_SPINDLE_INDEX_PULSE))  
  { // Process the detection of a spindle index pulse;
    if (settings.sync_pulses_per_revolution > 0)
    {                          // If index pulses are enabled.
      calculate_spindle_rpm(); // Process the pulse so the RPM will be updated in the real time status report
      if (spindle_synchronization_active())
      { // if spindle synchronization is active
        if bit_istrue (settings.status_report_mask, BITFLAG_RT_STATUS_SYNC_STATE)
        {                                                                    // if setting report mask is set for reporting the synchronization status in the real time status report
        ...
        if bit_istrue (settings.status_report_mask, BITFLAG_FEED_BACK_SYNC_STATUS)
        {                                                                              // if setting report mask is set for synchronization error feedback
      ...
      if (settings.sync_pulses_per_revolution == 1)
      { // Just an index pulse, emulate the receive of a sync pulse.
        process_spindle_synchronization_pulse();
      }
    }
    system_clear_threading_exec_flag(EXEC_SPINDLE_INDEX_PULSE);
  }
  if (bit_istrue(threading_exec_flags, EXEC_PLANNER_SYNC_PULSE))
  { // If a sync pulse was detected;
    if (spindle_synchronization_active())
    {
      update_planner_feed_rate();
    } // if spindle synchronization is active, update the planner to synchronize spindle
